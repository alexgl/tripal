   ...
   function tripal_set_dropable_box(){
      ...
      $('.tripal_expandableBox').click(
         function() {
            ...
            function(){
               // Get the base url, default base_url assume the Clean URL function is on
               // so just get the url substring till /node
               ...
               if($(this).css("display") == "none" ){               
                  $(this).prev().css("background-image", "url('" + baseurl + "/sites/all/modules/tripal_analysis_blast/images/arrow-down-48x48.png')");
                  $(this).prev().css("background-repeat","no-repeat");
                  $(this).prev().css("background-position","top right");
               } else {
                  $(this).prev().css("background-image", "url('" + baseurl + "/sites/all/modules/tripal_analysis_blast/images/arrow-up-48x48.png')");
                  $(this).prev().css("background-repeat","no-repeat");
                  $(this).prev().css("background-position","top right");
               }
            });
